...
class FilesizeformatterAPIClientError(Exception):
...
class ValidationError(FilesizeformatterAPIClientError):
...
class FilesizeformatterAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"bytes": {"type": "integer", "required": True, "min": 0}, "unit": {"type": "string", "required": False}, "precision": {"type": "integer", "required": False, "min": 0, "max": 10, "default": 2}, "standard": {"type": "string", "required": False, "default": "binary"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the FilesizeformatterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises FilesizeformatterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/filesizeformatter'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise FilesizeformatterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[FilesizeformatterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns FileSizeFormatterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises FilesizeformatterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise FilesizeformatterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise FilesizeformatterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise FilesizeformatterAPIClientError(f"Request failed: {str(e)}")
